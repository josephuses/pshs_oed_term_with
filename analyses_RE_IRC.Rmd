---
title: "Analysis of Readiness Examination Scores: IRC"
date: "10/30/2018"
output:
  word_document: 
...
* The 2018 MCQ RE scores for Grade 8 have an increasing trend. Notably, English scores hav a remarkable increase compared to 2017. On the other hand, the increasing trend of the distribution of Integrated Science scores was halted by a decrease in 2018.
* There are remarkable improvements in the Grade 8 non-MCQ RE scores for Filipino, IS, and Math. However, there is a noticeable downward shift in the Social Science non-MCQ scores. 
...
* In terms of the Grade 10 non-MCQ tests, the scores in Biology, English, and Filipino have an increasing trend. 
...
* Visible skewness in the distribution of scores can be found in the Grade 8 Filipino Non-MCQ scores. That is, more students got scores that are higher than the mean score than those who did not.
* In the following analyses, 50% is used to benchmark the scores of the students. There are two ways of viewing this benchmark: the subjects are easy or the students are not "ready" in more than 50% of the skills covered in the tests. These two views may not be mutually exclusive of each other. Also, the 50% benchmark is arbitrary and can be set by the subject-matter experts. However, there is the danger of setting the benchmark too low in order to make it appear that the students performed well when in fact they fail to show readiness in most skills required by the items that they fail to answer correctly[^1]. 
  ...
  - In terms of the non-MCQ tests for Grade 8, only 18% of the test-takers scored above 50% in Social Science, which is the lowest among all of the subjects. In the rest of the subjects, more than 50% of the students got scores that are abovec 50%. In particular, 84% of the test-takers got at least 50% of the items correctly in the Filipino non-MCQ test.
  - More than 50% of the Grade 10 test-takers scored 50% or above in all Grade 10 MCQ subjects except in Statistics (44%), Physics (15%) and Mathematics (40%). On the other hand, almost all test-takers got a score of more than 50% in Filipino (98%) and English (98%).
  - In all of the Grade 10 non-MCQ tests in STEM, below 50% of the test-takers got scores of at least 50%. More worrisome is the fact that in all of the computational subjects, less than 15% got scores above 50% (Math---6%; Stat---5%; Physics---11%). However, this is not surprising, as this has been the trend for these subjects in the past years.
...
```{r}
re2015_18 <- setDT(re2015_18)[,stype := case_when(subject %in% c("english", "filipino", "ss")~"Humanities",TRUE~"STEM")][campus == "IRC"]
```
