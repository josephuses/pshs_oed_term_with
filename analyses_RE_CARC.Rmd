---
title: "Analysis of Readiness Examination Scores: CARC"
date: "10/30/2018"
output:
...
* In terms of the Grade 10 non-MCQ tests, only English and Filipino have had a remarkable increase in the distribution of scores. The others either remained the same or have become lower than the previous year.
* In general, the scores for the MCQ tests are normally distributed. This means that, in the MCQ tests, about 68% of the students scored 1 standard deviation about the mean score. Visible skewness in the distribution of scores can be found in some of the Non-MCQ scores. Notably:
  ...
  - an appreciable number of students scored lower than the mean scores in the Grade 10 Chemistry, English and Math Non-MCQ tests.
* In the following analyses, 50% is used to benchmark the scores of the students. There are two ways of viewing this benchmark: the subjects are easy or the students are not "ready" in more than 50% of the skills covered in the tests. These two views may not be mutually exclusive of each other. Also, the 50% benchmark is arbitrary and can be set by the subject-matter experts. However, there is the danger of setting the benchmark too low in order to make it appear that the students performed well when in fact they fail to show readiness in most skills required by the items that they fail to answer correctly[^1]. 
  - In terms of the MCQ test scores for Grade 8, more than half of the students who took the tests got more than 50% of the items correctly in all tests. However, more or less 2 out of 5 students scored below 50% in Integrated Science. This is followed by Math, in which 21% of the students scored below 50%. On the other hand, 97% of the students scored above 50% in English.
  - In terms of the non-MCQ tests for Grade 8, only 26% of the test-takers scored above 50% in Social Science, which is the lowest among all of the subjects. On the other extreme side, 95% of the test-takers got at least 50% of the items correctly in the Filipino non-MCQ test.
  ...
  - In all the non-MCQ tests in STEM, except for Biology, the test-takers got scores below 50%. However, even in Biology, there were only 51% of the test-takers who scored more than 50%. More worrisome is the fact that in all of the computational subjects, less than 40% got scores above 50% (Math---14%; Stat---1%; Physics---37%). However, this is not surprising as this has been the trend for these subjects in the past years.
...
```{r}
re2015_18 <- setDT(re2015_18)[,stype := case_when(subject %in% c("english", "filipino", "ss")~"Humanities",TRUE~"STEM")][campus == "CARC"]
```
