---
title: "Analysis of Readiness Examination Scores: CMC"
date: "10/30/2018"
output:
  word_document: 
...
* The Grade 10 MCQ scores for English and Statistics follow an increasing trend. There are remarkable increases in the scores for Biology, Filipino and Social Science. On the other hand, there is an appreciable decrease in the Physics scores. Such decrease is also the trend, system-wide.
* In terms of the Grade 10 non-MCQ tests, English, Filipino, Math, and Social Science have had a remarkable increase in the distribution of scores. The others either remained the same or have become lower than the previous year.
* In general, the scores for the MCQ tests are normally distributed, except for Filipino, in which noticeably more students scored higher than the campus mean. This means that, in the MCQ tests, about 68% of the students scored 1 standard deviation about the mean score. 
* Visible skewness in the distribution of scores can be found in some of the Non-MCQ scores. Notably:
  ...
  - in Grade 8 Integrated Science, Mathematics, and Social Science non-MCQ test scores, more students scored lower than the corresponding campus mean;
  - an appreciable number of students scored higher than the mean scores in the Grade 10 Chemistry;
  - a significant number of students scored lower in Social Science than the campus mean score.
* In the following analyses, 50% is used to benchmark the scores of the students. There are two ways of viewing this benchmark: the subjects are easy or the students are not "ready" in more than 50% of the skills covered in the tests. These two views may not be mutually exclusive of each other. Also, the 50% benchmark is arbitrary and can be set by the subject-matter experts. However, there is the danger of setting the benchmark too low in order to make it appear that the students performed well when in fact they fail to show readiness in most skills required by the items that they fail to answer correctly[^1]. 
  ...
  - In terms of the non-MCQ tests for Grade 8, only 24% of the test-takers scored above 50% in Social Science, which is the lowest among all of the subjects. On the other hand, 94% of the test-takers got at least 50% of the items correctly in the Filipino non-MCQ test.
  - More than 50% of the Grade 10 test-takers scored 50% or above in all Grade 10 MCQ subjects except in Statistics (27%), Physics (30%) and Mathematics (45%). On the other hand, almost all test-takers got a score of more than 50% in English (94%) and Filipino (100%).
  - In all of the Grade 10 non-MCQ tests in STEM, except for Biology, at least 50% of the test-takers got scores of at least 50%. In Biology, there were only 21% of the test-takers scored more than 50%. More worrisome is the fact that in all of the computational subjects, less than 15% got scores above 50% (Math---7%; Stat---2%; Physics---11%). However, this is not surprising, as this has been the trend for these subjects in the past years.
...
```{r}
re2015_18 <- setDT(re2015_18)[,stype := case_when(subject %in% c("english", "filipino", "ss")~"Humanities",TRUE~"STEM")][campus == "CMC"]
```
