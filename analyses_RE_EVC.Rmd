---
title: "Analysis of Readiness Examination Scores: EVC"
date: "10/30/2018"
output:
  word_document: 
...
* The MCQ RE scores for Grade 8 continue to improve in general with the exception of Integrated Science and Filipino, where the scores are lower compared to the 2017 scores.
* There are remarkable improvements in the Grade 8 non-MCQ RE scores for Filipino, IS, and Math. However, there is a noticeable downward shift in the Social Science non-MCQ scores. Such drop in scores in Social Science is quite remarkable.
* The Grade 10 MCQ scores follow an increasing trend in all subjects except in Physics. There is a remarkable decrease of scores in Physics. Such decrease is also the trend, system-wide.
* In terms of the Grade 10 non-MCQ tests, the scores in English, Filipino, Math, and Physics have an increasing trend. There is also a significant increase in the Biology scores.
...
* Visible skewness in the distribution of scores can be found in some of the Non-MCQ scores. Notably:
  - in Grade 8 and 10 Filipino Non-MCQ tests, more students got scores that are higher than the mean score; the same is also true for Grade 10 English non-MCQ test;
  - in Grade 10 Biology non-MCQ test most students scored less than the campus mean.
* In the following analyses, 50% is used to benchmark the scores of the students. There are two ways of viewing this benchmark: the subjects are easy or the students are not "ready" in more than 50% of the skills covered in the tests. These two views may not be mutually exclusive of each other. Also, the 50% benchmark is arbitrary and can be set by the subject-matter experts. However, there is the danger of setting the benchmark too low in order to make it appear that the students performed well when in fact they fail to show readiness in most skills required by the items that they fail to answer correctly[^1]. 
  ...
  - In terms of the non-MCQ tests for Grade 8, only 20% of the test-takers scored above 50% in Social Science, which is the lowest among all of the subjects. On the other hand, 98% of the test-takers got at least 50% of the items correctly in the Filipino non-MCQ test.
  - More than 50% of the Grade 10 test-takers scored 50% or above in all Grade 10 MCQ subjects except in Statistics (48%), Physics (13%) and Mathematics (42%). On the other hand, almost all test-takers got a score of more than 50% in English (96%) and while all got at least 50% in Filipino.
  - In all of the Grade 10 non-MCQ tests in STEM, below 50% of the test-takers got scores of at least 50%. More worrisome is the fact that in all of the computational subjects, less than 20% got scores above 50% (Math---0%; Stat---3%; Physics---16%). However, this is not surprising, as this has been the trend for these subjects in the past years.
...
```{r}
re2015_18 <- setDT(re2015_18)[,stype := case_when(subject %in% c("english", "filipino", "ss")~"Humanities",TRUE~"STEM")][campus == "EVC"]
```
